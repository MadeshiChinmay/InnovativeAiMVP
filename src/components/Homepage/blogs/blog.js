...
var showingAll = [];
var currentUsername;
var emptyString = "There's no content here yet, but check back soon!";
...
$(document).ready(function() {
    if (document.getElementById('blogContent') && !document.getElementById('postSection')) {
        if (window.self == window.top) {
            ...
            emptyString = "Posts don't show up in the editor.";
            addEmptyBlog();
        }
...
var displayPost = function(i) {
    ...
    var dateText = document.createTextNode("Posted on " + blogInfo.posts[i].posted);
...
var addComments = function(div, post) {
    ...
    if (iStart !== 0) {
        ...
        var viewText = document.createTextNode('Show All');
        ...
        var viewMore = document.createElement('a');
        var viewText = document.createTextNode('Hide');
        viewMore.appendChild(viewText);
        viewMore.className = 'viewMoreComments';
        viewMore.setAttribute('name', postId);
        viewMore.onclick = function() {
    ...
    commentSection.setAttribute('placeholder', 'Write a comment...');
...
var sendComment = function(postId) {
    if (document.getElementById(postId).getElementsByClassName('commentSection')[0].value === '') {
        swal('Uh Oh!', 'You need to enter some text in order to comment.', 'error');
    ...
    if (blogInfo.thisUser.username === "") {
        ...
        $.ajax({
            ...
            error: function(err) {
                ...
                swal({
                    title: "Uh Oh!",
                    text: "There seems to be an issue posting this comment. Please try again later.",
                    type: "error",
...
var checkUsername = function(username) {
    ...
    $.ajax({
        ...
        error: function(err) {
            ...
            swal({
                title: "Uh Oh!",
                text: "There seems to be an issue checking this username. Please try again later.",
                type: "error",
                showCancelButton: false,
                confirmButtonText: "Ok",
                closeOnConfirm: true
...
var getPassword = function(username) {
    swal({
        title: "Password",
        text: 'This username already exists. If it\'s you, please enter the password for it:',
        type: 'input',
        showCancelButton: true,
        closeOnConfirm: true
    }, function(inputValue) {
...
var setPassword = function(username) {
    swal({
        title: "Password",
        text: 'Choose a password for your username:',
        type: 'input',
        showCancelButton: true,
        closeOnConfirm: true
    }, function(inputValue) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data === "success") {
                ...
                swal('Success!', 'You\'re signed in! Go ahead and post your comment.', 'success');
            } else {
                swal('Uh Oh!', data, 'error');
                document.body.className = '';
            }
